...
export class TestController {
  ...
  @httpGet('/')
  public async getTestData(@request() req: express.Request, @response() res: express.Response, @next() nxt: NextFunction) {
    const result = await this.dbService.find('test', {});
    result ? res.status(200).send(result) : nxt(new InformationNotFound());
  }

  @httpPost('/')
  public async postTestData(@request() req: express.Request, @response() res: express.Response, @next() nxt: NextFunction) {
    const result = await this.dbService.createOne('test', req.body);
    result ? res.status(200).send(result) : nxt(new InformationNotFound());
  }

  @httpPost('/multiple')
  public async postTestDataMultiple(@request() req: express.Request, @response() res: express.Response, @next() nxt: NextFunction) {
    const result = await this.dbService.createMany('test', req.body);
    result ? res.status(200).send(result) : nxt(new InformationNotFound());
  }

  @httpGet('/:id')
  public async getOneTestData(@request() req: express.Request, @response() res: express.Response, @next() nxt: NextFunction) {
  ...
  @httpPatch('/:id')
  public async patchTestData(@request() req: express.Request, @response() res: express.Response, @next() nxt: NextFunction) {
    const locator = { _id: this.utilService.objectifyId(req.params.id) };
