...
export class DatabaseService {
  ...
  public async updateOne(collection: string, locator: IDbLocator, data: object): Promise<any> {
    return await this.dbConnection.collection(collection).findOneAndUpdate(locator, {$set: data}, { upsert: false, returnOriginal: false });
  }
  ...
  public async deleteMany(collection: string, locatorMultiple: IDbLocatorMuliple): Promise<any> {
    return await this.dbConnection.collection(collection).deleteMany(locatorMultiple);
  }
}
